...
from envoy.dependency.cve_scan import ACPE, CPEError


@abstracts.implementer(ACPE)
class DummyCPE:
    pass
...
def test_cpe_constructor(product, version):
    ...
    cpe = DummyCPE("PART", "VENDOR", **kwargs)
...
def test_cpe_cls_fromstring(text):
    ...
    if len(components) < 6:
        ...
        raises = True
    if raises:
        with pytest.raises(CPEError) as e:
            ACPE.from_string(text)
        assert (
            e.value.args[0]
            == f"CPE string ({text}) must be a valid CPE v2.3 string")
        return
    cpe = ACPE.from_string(text)
