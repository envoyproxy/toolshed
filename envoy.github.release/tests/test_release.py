...
async def test_release_assets_url(patches):
    ...
    with patched as (m_release, ):
        _release = AsyncMock()
        m_release.side_effect = _release
        assert (
...
async def test_release_exists(patches, version):
    ...
    with patched as (m_release, m_name):
        ...
        m_release.side_effect = AsyncMock(return_value=_versions)
        assert await release.exists == (version in _versions)
...
async def test_release_release_names(patches):
    ...
    _release_names = [dict(tag_name=f"TAG{i}") for i in range(0, 3)]
...
async def test_release_upload_url(patches):
    ...
    with patched as (m_release, ):
        _release = AsyncMock()
        m_release.side_effect = _release
        split = _release.return_value.__getitem__.return_value.split
        assert (
...
async def test_release_fetch(patches, asset_types, errors):
    ...
    async def _fetcher(_releaser, path, asset_types, append=False):
