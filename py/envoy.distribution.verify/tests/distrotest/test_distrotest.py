...
def test_config_config_path(config_path):
    ...
    assert (
        config.config_path
        == config_path or distrotest.distrotest.DISTROTEST_CONFIG_PATH)
...
def test_config_install_img_path(patches):
    config = distrotest.DistroTestConfig(
        "DOCKER", "PATH", "TARBALL", "INSTALL", "INSTALL",
        "MAINTAINER", "VERSION")
    patched = patches(
        "pathlib",
        prefix="envoy.distribution.distrotest.distrotest")
...
def test_config_get_package_type(patches, pkg_type, pkg_types):
    ...
    with patched as (m_name, m_items):
        ...
        m_items.return_value = [
            ("X", dict(images=["TYPE6", "TYPE7"])),
            ("Y", dict(images=pkg_types))]
...
def test_image_constructor(patches, stream):
    ...
    assert image.prefix == distrotest.distrotest.DOCKER_IMAGE_PREFIX
    assert "prefix" not in image.__dict__
    assert (
        image.dockerfile_template
        == distrotest.distrotest.DOCKERFILE_TEMPLATE)
...
def test_image_build_command(patches):
    ...
    with patched as (m_config, ):
        config_item = (
            m_config.return_value.__getitem__.return_value.__getitem__)
        strip = config_item.return_value.strip
        assert (
            image.build_command
...
def test_image_dockerfile(patches):
    image = distrotest.DistroTestImage("CONFIG", "BUILD_IMAGE", "NAME")
    patched = patches(
        ("DistroTestImage.build_command",
         dict(new_callable=PropertyMock)),
        ("DistroTestImage.env",
         dict(new_callable=PropertyMock)),
        ("DistroTestImage.dockerfile_template",
         dict(new_callable=PropertyMock)),
        ("DistroTestImage.ctx_install_dir",
         dict(new_callable=PropertyMock)),
        ("DistroTestImage.keyfile",
         dict(new_callable=PropertyMock)),
        ("DistroTestImage.keyfile_img_path",
         dict(new_callable=PropertyMock)),
        ("DistroTestImage.install_img_path",
         dict(new_callable=PropertyMock)),
        ("DistroTestImage.testfile",
         dict(new_callable=PropertyMock)),
        ("DistroTestImage.testfile_img_path",
         dict(new_callable=PropertyMock)),
        prefix="envoy.distribution.distrotest.distrotest")

    with patched as patchy:
        (m_command, m_env, m_template, m_install,
         m_kfile, m_kpath, m_minstall, m_tfile, m_tpath) = patchy
...
def test_image_add_dockerfile(patches):
    ...
    image = distrotest.DistroTestImage(
        "CONFIG", "BUILD_IMAGE", "NAME", stream=stream)
    patched = patches(
        "shutil",
...
async def test_image_build(patches, raises):
    image = distrotest.DistroTestImage(
        "CONFIG", "BUILD_IMAGE", "NAME", "STREAM")
    patched = patches(
    ...
    with patched as (m_build, m_add, m_stream, m_docker, m_path, m_tag):
        if raises:
            build_error = docker_utils.BuildError("AN ERROR OCCURRED", "X")
            m_build.side_effect = build_error
            ...
            assert (
                e.value.args
                == build_error.args)
            assert e.value.__cause__ is build_error
        else:
...
def test_image_get_environment(patches, items):
    ...
    with patched as (m_dict, m_get, m_path, m_config):
        ...
        (m_config.return_value.__getitem__
         .return_value.items.return_value) = _items
        assert (
            image.get_environment(
                "PKG_FNAME", "PKG_NAME", "DISTRO_NAME")
            == _dict)
...
def test_image_installable_img_path(patches):
    ...
    with patched as (m_dir, ):
        assert (
            image.installable_img_path("INSTALLABLE")
...
def test_distrotest_constructor(patches):
    ...
    assert dtest.prefix == distrotest.distrotest.DOCKER_CONTAINER_PREFIX
...
def test_distrotest_handle_test_error(patches, msg):
    ...
    with patched as (m_error, m_stdout):
        assert not dtest.handle_test_error(msg)
...
@pytest.mark.parametrize(
    "msg, raw_line",
    [("MESSAGE", "MESSAGE"),
     ("[TESTRUN] MESSAGE", "[TESTRUN] MESSAGE"),
     ("[TESTRUNTESTNAME", "[TESTRUNTESTNAME"),
     ("", "")])
def test_distrotest_handle_test_error_malformed(patches, msg, raw_line):
    check = checker.Checker()
    dtest = distrotest.DistroTest(
        check, "CONFIG", "NAME", "IMAGE", "INSTALLABLE")
    patched = patches(
    ...
    assert dtest._failures == ['malformed-test-output']
    assert m_stdout.return_value.error.call_count == 0
    assert m_error.call_count == 1
    error_msg = m_error.call_args[0][0][0]
    assert "malformed control line" in error_msg
    assert repr(raw_line) in error_msg
...
def test_distrotest_run_message(patches, testname):
    ...
    if testname:
        assert (
            dtest.run_message("MESSAGE", testname)
            == f"[NAME/{testname}] MESSAGE")
    else:
...
async def test_distrotest_start(patches, running):
    ...
    with patched as (m_log, m_msg, m_create, m_logs, m_name):
        (m_create.return_value.show
         .return_value.__getitem__.return_value
         .__getitem__.return_value) = running
...
async def test_distrotest_cleanup_cancelled_error(patches):
    check = checker.Checker()
    dtest = distrotest.DistroTest(
        check, "CONFIG", "NAME", "IMAGE", "INSTALLABLE")
    patched = patches(
...
@pytest.mark.parametrize(
    "build_raises",
    [None,
     distrotest.ConfigurationError,
     distrotest.BuildError,
     aiodocker.exceptions.DockerError,
     Exception])
@pytest.mark.parametrize(
    "start_raises",
    [None,
     distrotest.ContainerError,
     aiodocker.exceptions.DockerError,
     Exception])
@pytest.mark.parametrize(
    "exec_raises",
