...
def test_abstract_changelogs_write_current(iters, patches, entries_layout):
    ...
    if entries_layout:
        ...
        assert (
            m_dir_path.return_value.__truediv__.call_args
            == [(".gitkeep", ), {}])
        gitkeep_path = m_dir_path.return_value.__truediv__.return_value
        assert (
            gitkeep_path.write_text.call_args
            == [("", ), {}])
...
def test_abstract_changelogs_write_version(patches, exists, entries_layout):
    ...
    if entries_layout:
        assert (
            ...
            == [(m_path.return_value, m_dir_path.return_value), {}])
        entries_rv = (
...
def test_abstract_changelog_get_data_from_entries_happy_path(patches):
    ...
    assert (
        m_yaml.call_args
        == [(yaml_path, m_typing.ChangelogSourceDict), {}])
    assert (
        m_yaml.return_value.__getitem__.call_args
        == [("date", ), {}])
    assert (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        exceptions.TypeCastingError])
def test_abstract_changelog_get_data_from_entries_missing_yaml(
        patches, raises):
    patched = patches(
        "utils.from_yaml",
        "typing",
    ...
    with patched as (m_yaml, m_typing):
        ...
        if raises in (yaml.reader.ReaderError, exceptions.TypeCastingError):
            with pytest.raises(exceptions.ChangelogParseError) as e:
                abstract.AChangelog.get_data_from_entries(yaml_path, entry_dir)
        else:
    ...
    assert (
        m_yaml.call_args
        == [(yaml_path, m_typing.ChangelogSourceDict), {}])
    assert not entry_dir.glob.called
    if raises in (yaml.reader.ReaderError, exceptions.TypeCastingError):
        assert (
            e.value.args[0]
            == ("Failed to parse: "
