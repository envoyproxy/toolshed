...
# ---------------------------------------------------------------------------
# parse_options – py_modules
# ---------------------------------------------------------------------------

def test_parse_options_py_modules():
    config = make_config("""
        [options]
        py_modules = my_module
    """)
    assert parse_options(config, "/x")["py_modules"] == ["my_module"]


def test_parse_options_py_modules_absent():
    config = make_config("[options]\n")
    assert "py_modules" not in parse_options(config, "/x")


# ---------------------------------------------------------------------------
# parse_options – packages explicit list
# ---------------------------------------------------------------------------

def test_parse_options_packages_explicit():
    config = make_config("""
        [options]
        packages =
            mypkg
            mypkg.subpkg
    """)
    assert parse_options(config, "/x")["packages"] == ["mypkg", "mypkg.subpkg"]


# ---------------------------------------------------------------------------
# parse_options – packages = find:
# ---------------------------------------------------------------------------

def test_parse_options_packages_find(tmp_path):
    ...
    config = make_config("""
        ...
        include =
            ...
            mypkg.*
    """)
    result = parse_options(config, str(tmp_path))
    assert "mypkg" in result["packages"]


def test_parse_options_packages_find_with_exclude(tmp_path):
    (tmp_path / "mypkg").mkdir()
    (tmp_path / "mypkg" / "__init__.py").touch()
...
def test_parse_options_packages_find_namespace(tmp_path):
    ns = tmp_path / "myns" / "subpkg"
    ns.mkdir(parents=True)
    (ns / "__init__.py").touch()
    config = make_config("""
        [options]
...
def test_parse_packages_find_include_and_exclude():
    config = make_config("""
        [options.packages.find]
        include =
            mypkg
        ...
        exclude =
            ...
            tests.*
    """)
    result = _parse_packages_find(config)
    assert result["include"] == ["mypkg", "mypkg.*"]
    assert result["exclude"] == ["tests", "tests.*"]


def test_parse_packages_find_no_section():
    config = make_config("[metadata]\nname=x\n")
    assert _parse_packages_find(config) == {}


def test_parse_packages_find_include_only():
    config = make_config("""
        [options.packages.find]
        include = mypkg
    """)
...
def test_full_abstracts_config(tmp_path):
    ...
    (tmp_path / "abstracts" / "__init__.py").touch()
    (tmp_path / "abstracts" / "py.typed").touch()
    ...
    assert "abstracts" in options_kwargs["packages"]
