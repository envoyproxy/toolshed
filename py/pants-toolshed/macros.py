...
def _pinned_req_names() -> set:
    """Canonical names of every target generated by //py/deps:reqs."""
    names = set()
    for raw_line in open(
            "py/deps/requirements.in", encoding="utf-8").read().splitlines():
        stripped = raw_line.strip()
        if not stripped or stripped.startswith("#") or stripped.startswith("-"):
            continue
        names.add(_req_bare_name(stripped))
    return names
...
def _setup_cfg_extras_require(namespace: str) -> list:
    """Flat list of all req strings in every [options.extras_require] group."""
    reqs = []
    in_extras = False
    in_group = False
    ...
    for raw_line in open(setup_cfg, encoding="utf-8").read().splitlines():
        ...
        if stripped.startswith("[") and stripped.endswith("]"):
            in_extras = (stripped == "[options.extras_require]")
            in_group = False
            continue
        if not in_extras:
            continue
        if not stripped or stripped.startswith("#"):
            continue
        # Group header e.g. `test =` or `lint = flake8>=6.1.0`
        if raw_line and not raw_line[0].isspace():
            ...
            if "=" in raw_line:
                ...
                if value:
                    reqs.append(value)
            continue
        # Indented continuation of current group
...
def toolshed_package(
    ...
    pinned = _pinned_req_names()
    all_reqs = (
        _setup_cfg_install_requires(namespace)
        + _setup_cfg_extras_require(namespace))
    excluded_reqs = [
        f"!!//py/deps:reqs#{name}"
        for name in {_req_bare_name(r) for r in all_reqs}
        if name in pinned]
