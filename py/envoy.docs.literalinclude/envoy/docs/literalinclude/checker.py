...
class LiteralIncludeDirective:
    ...
    def _parse_line_spec(spec: str) -> list[int]:
        """Parse a line specification like '1-10,15,20-25' into individual line numbers."""
        lines = []
        parts = spec.split(',')
...
class LiteralIncludeChecker:
    ...
    EMPHASIZE_LINES_RE = re.compile(r'^\s+:emphasize-lines:\s+(.+?)$', re.MULTILINE)
    ...
    def find_rst_files(self, search_dirs: Optional[list[str]] = None) -> list[Path]:
        """Find all RST files in specified directories.
        
        Args:
            search_dirs: List of directory names to search (default: ['docs', 'api'])
    ...
    def parse_rst_file(self, rst_file: Path) -> list[LiteralIncludeDirective]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if match:
                ...
                source_file = self._resolve_source_path(rst_file, source_path_str)
    ...
    def _resolve_source_path(self, rst_file: Path, source_path_str: str) -> Path:
    ...
    def get_file_changes_since(self, file_path: Path, since_commit: str) -> Optional[dict]:
        ...
        Returns:
            Dictionary with 'added_lines' and 'removed_lines' lists of line numbers,
            or None if unable to get changes
        """
        try:
            # Get the diff with line numbers
            result = subprocess.run(
                ['git', 'diff', '--unified=0', since_commit, 'HEAD', '--', str(file_path)],
                cwd=self.repo_root,
                capture_output=True,
                text=True,
                check=True
    ...
    def check_directive_outdated(self, directive: LiteralIncludeDirective) -> bool:
        ...
        if not directive.rst_file.exists() or not directive.source_file.exists():
            return False
        ...
        try:
            ...
            result = subprocess.run(
                ['git', 'log', '--format=%H', '--', str(directive.source_file)],
                cwd=self.repo_root,
                capture_output=True,
                text=True,
                check=True
            ...
            # Simple heuristic: if source file has commits after the RST's last commit
            if source_commits and rst_commits:
                rst_last_idx = len(source_commits)  # Assume all source commits are after
                try:
                ...
                if rst_last_idx > 0:
                    ...
                    if changes:
                        ...
                        if max_line:
                            ...
                            for line_num in changes['added_lines'] + changes['removed_lines']:
        ...
        for rst_file in rst_files:
            ...
            for directive in directives:
                if self.check_directive_outdated(directive):
                    reason = (
                        f"Source file '{directive.source_file.name}' was modified "
                        f"after RST file '{directive.rst_file.name}' and changes "
