...
class LiteralIncludeFixer:
    ...
    def calculate_correct_lines(
        ...
        This attempts to determine what the line numbers should be based on
        ...
        Returns:
            Dictionary with 'lines' and/or 'emphasize_lines' keys with corrected specs,
            or None if unable to determine correct lines
        """
    ...
    def fix_directive(
        ...
        try:
            ...
            modified = False
            for i in range(directive.rst_line_number - 1, min(directive.rst_line_number + 20, len(lines))):
                line = lines[i]
                
                if 'lines' in corrections and ':lines:' in line:
                    old_match = re.match(r'(\s+):lines:\s+(.+?)$', line)
                    if old_match:
                        indent = old_match.group(1)
                        lines[i] = f"{indent}:lines: {corrections['lines']}"
                        modified = True
                
                if 'emphasize_lines' in corrections and ':emphasize-lines:' in line:
                    old_match = re.match(r'(\s+):emphasize-lines:\s+(.+?)$', line)
                    if old_match:
                        indent = old_match.group(1)
                        lines[i] = f"{indent}:emphasize-lines: {corrections['emphasize_lines']}"
                        modified = True
