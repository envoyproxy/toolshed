...
def test_cve_cpes(patches):
    ...
    patched = patches(
        "set",
        ("ADependencyCVE.nodes", dict(new_callable=PropertyMock)),
        "ADependencyCVE.gather_cpes",
        prefix="envoy.dependency.check.abstract.cves.cve")
...
def test_cve_description():
    cve = DummyDependencyCVE(MagicMock(), "TRACKED_CPES")
    assert (
        cve.description
...
def test_cve_dependency_match(patches, matches):
    cve = DummyDependencyCVE("CVE", "TRACKED CPES")
    patched = patches(
        ("ADependencyCVE.cpes",
         dict(new_callable=PropertyMock)),
        "ADependencyCVE.wildcard_version_match",
        prefix="envoy.dependency.check.abstract.cves.cve")
    dep = MagicMock()
    cpes = []
...
def test_cve_gather_cpes(patches, include):
    cve = DummyDependencyCVE("CVE", "TRACKED_CPES")
    patched = patches(
        ("ADependencyCVE.cpe_class", dict(new_callable=PropertyMock)),
        "ADependencyCVE.include_version",
        prefix="envoy.dependency.check.abstract.cves.cve")
    nodes = [
        dict(),
        dict(
...
def test_cve_include_version(patches, tracked, matches):
    ...
    cve = DummyDependencyCVE("CVE", tracked_cpes)
    patched = patches(
        ("ADependencyCVE.version_matcher_class",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.cves.cve")
    cpe_match = MagicMock()
    cpe = MagicMock()
