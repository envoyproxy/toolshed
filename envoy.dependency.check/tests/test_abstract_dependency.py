...
from envoy.dependency.check import AGithubDependency, exceptions


@abstracts.implementer(AGithubDependency)
class DummyGithubDependency:
...
class DummyGithubDependency2(DummyGithubDependency):
...
def test_dependency_constructor(patches, github_url):
    patched = patches(
        ("AGithubDependency.github_url",
         dict(new_callable=PropertyMock)),
        ("AGithubDependency.urls",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")

    with patched as (m_gh_url, m_urls):
        m_gh_url.return_value = github_url
        m_urls.return_value = [f"URL{i}" for i in range(0, 3)]

        with pytest.raises(TypeError):
            AGithubDependency("ID", "METADATA", "GITHUB")

        if not github_url:
            with pytest.raises(exceptions.NotGithubDependency) as e:
                DummyGithubDependency("ID", "METADATA", "GITHUB")
            assert (
                e.value.args[0]
                == 'ID is not a GitHub repository\nURL0\nURL1\nURL2')
            return
        dependency = DummyGithubDependency("ID", "METADATA", "GITHUB")

    assert not m_urls.called
...
def test_dependency_dunder_gt(id, other_id):
    dependency1 = DummyGithubDependency2(id, "METADATA", "GITHUB")
    dependency2 = DummyGithubDependency2(other_id, "METADATA", "GITHUB")
...
def test_dependency_dunder_str(patches):
    dependency = DummyGithubDependency2("ID", "METADATA", "GITHUB")
    patched = patches(
        ("AGithubDependency.version",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
...
async def test_dependency_commits_since_current(patches, count):
    dependency = DummyGithubDependency2("ID", "METADATA", "GITHUB")
    patched = patches(
        ("AGithubDependency.release",
         dict(new_callable=PropertyMock)),
        ("AGithubDependency.repo",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
    ...
    assert (
        getattr(
            dependency,
            AGithubDependency.commits_since_current.cache_name)[
...
def test_dependency_github_version(patches, archive, endswith):
    dependency = DummyGithubDependency2("ID", "METADATA", "GITHUB")
    patched = patches(
        ("AGithubDependency.url_components",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
    component = MagicMock()
...
def test_dependency_github_version_name(patches, tagged):
    dependency = DummyGithubDependency2("ID", "METADATA", "GITHUB")
    patched = patches(
        ("AGithubDependency.github_version",
         dict(new_callable=PropertyMock)),
        ("AGithubDependency.release",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
...
async def test_dependency_has_recent_commits(patches, commits):
    dependency = DummyGithubDependency2("ID", "METADATA", "GITHUB")
    patched = patches(
        ("AGithubDependency.recent_commits",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
    ...
    assert not getattr(
        dependency,
        AGithubDependency.has_recent_commits.cache_name,
        None)
...
async def test_dependency_newer_release(patches, newest):
    ...
    patched = patches(
        ...
        ("AGithubDependency.release",
         dict(new_callable=PropertyMock)),
        ("AGithubDependency.release_class",
         dict(new_callable=PropertyMock)),
        ("AGithubDependency.repo",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
    ...
    assert (
        getattr(
            dependency,
            AGithubDependency.newer_release.cache_name)[
                "newer_release"]
        == result)
...
async def test_dependency_recent_commits(patches, tagged):
    dependency = DummyGithubDependency2("ID", "METADATA", "GITHUB")
    patched = patches(
        ("AGithubDependency.commits_since_current",
         dict(new_callable=PropertyMock)),
        ("AGithubDependency.release",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
    ...
    assert (
        getattr(
            dependency,
            AGithubDependency.recent_commits.cache_name)[
                "recent_commits"]
        == result)
...
async def test_dependency_release_date_mismatch(patches, date1, date2):
    ...
    patched = patches(
        ("AGithubDependency.release",
         dict(new_callable=PropertyMock)),
        ("AGithubDependency.release_date",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
    ...
    assert not getattr(
        dependency,
        AGithubDependency.release_date_mismatch.cache_name,
        None)
...
def test_dependency_repo(patches):
    ...
    dependency = DummyGithubDependency2("ID", "METADATA", github)
    patched = patches(
        ("AGithubDependency.organization",
         dict(new_callable=PropertyMock)),
        ("AGithubDependency.project",
         dict(new_callable=PropertyMock)),
        prefix="envoy.dependency.check.abstract.dependency")
