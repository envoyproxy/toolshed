...
def test_issue_dep(patches):
    ...
    assert (
        list(m_parsed.return_value.get.call_args)
        == [("dep", ), {}])
...
def test_issue_parsed(patches, parsed):
    ...
    assert (
        list(m_re.return_value.search.call_args)
    ...
    assert (
        list(list(c)
             for c
             in m_re.return_value.search.return_value.group.call_args_list)
        == [[(1, ), {}], [(2, ), {}]])
...
async def test_issue_close():
    ...
    assert (
        list(mock_issue.close.call_args)
        == [(), {}])
...
async def test_issue_close_old(patches):
    ...
    assert (
        list(old_issue.comment.call_args)
        == [(m_tpl.return_value.format.return_value, ), {}])
    assert (
        list(m_tpl.return_value.format.call_args)
        == [(),
    ...
    assert (
        list(old_issue.close.call_args)
        == [(), {}])
...
async def test_issue_comment():
    ...
    assert (
        list(mock_issue.comment.call_args)
        == [("COMMENT", ), {}])
...
def test_issues_title_re(patches):
    ...
    assert (
        list(title_re_tpl.format.call_args)
...
async def test_issues_create(patches, in_titles):
    ...
    assert (
        list(m_repo.return_value.issues.create.call_args)
        == [(m_title.return_value,),
            dict(body=m_body.return_value, labels=["LABEL"])])
    assert (
        list(m_body.call_args)
        == [(dep, ), {}])
...
async def test_issues_issue_body():
    ...
    assert (
        list(body_tpl.format.call_args)
        == [(),
            dict(dep=dep,
...
async def test_issues_issue_title():
    ...
    assert (
        list(title_tpl.format.call_args)
        == [(),
            dict(dep=dep,
                 newer_release=newer_release.return_value,
                 title_prefix="TITLE_PREFIX")])
...
def test_issues_iter_issues(patches):
    ...
    assert (
        list(m_repo.return_value.issues.search.call_args)
        == [(issues_search_tpl.format.return_value, ), {}])
    assert (
        list(issues_search_tpl.format.call_args)
        == [(), dict(self=issues)])
