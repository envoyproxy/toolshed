...
# Cross-compilation libc++ libraries (libcxx_cross)

This package provides a way to obtain prebuilt `libc++.a`, `libc++abi.a`, and
`libunwind.a` from the official LLVM release tarballs for use when
cross-compiling. The libraries are extracted directly from the upstream LLVM
release — no local compilation required.

This keeps the sysroot lean (glibc headers/libs only) while still making the
libc++ static libraries available as a separate Bazel repository.

## Using with WORKSPACE

```starlark
load("@envoy_toolshed//compile:libcxx_cross.bzl", "setup_libcxx_cross")

setup_libcxx_cross()  # Defaults to aarch64, LLVM 18.1.8
```
...
This creates a `@libcxx_cross` repository with the following targets:

- `@libcxx_cross//:libcxx` — `libc++.a`
- `@libcxx_cross//:libcxxabi` — `libc++abi.a`
- `@libcxx_cross//:libunwind` — `libunwind.a`
- `@libcxx_cross//:libcxx_cross` — all three bundled together
- `@libcxx_cross//:config_site_header` — `__config_site` header filegroup
