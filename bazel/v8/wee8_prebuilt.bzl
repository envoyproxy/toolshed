...
_V8_ALIAS_BUILD = """\
# Auto-generated by envoy_toolshed//v8:wee8_prebuilt.bzl
# This repository provides a @v8//:wee8 alias to the prebuilt wee8 library.
package(default_visibility = ["//visibility:public"])

alias(
    name = "wee8",
    actual = "@envoy_toolshed//v8:wee8",
)
"""

def _v8_prebuilt_alias_impl(ctx):
    """Create a @v8 repository that aliases to envoy_toolshed prebuilt wee8."""
    ctx.file("BUILD.bazel", _V8_ALIAS_BUILD)
    ctx.file("WORKSPACE", "")

_v8_prebuilt_alias = repository_rule(
    implementation = _v8_prebuilt_alias_impl,
    doc = "Creates a @v8 repository alias to use prebuilt wee8",
)

def v8_prebuilt_alias():
    """Create @v8 repository that aliases to envoy_toolshed prebuilt wee8.

    This allows external dependencies (like proxy_wasm_cpp_host) that reference
    @v8//:wee8 to automatically use the prebuilt library instead of building from source.
    """
    _v8_prebuilt_alias(name = "v8")
