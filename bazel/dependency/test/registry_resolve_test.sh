...
# NB: do not assign RUNFILES_DIR here; the git launcher (and other runfiles
# aware tools) inherit it and use it as the runfiles root.
if [ -n "${TEST_SRCDIR:-}" ]; then
    if [ -d "${TEST_SRCDIR}/envoy_toolshed" ]; then
        WORKSPACE_DIR="${TEST_SRCDIR}/envoy_toolshed"
    else
        WORKSPACE_DIR="${TEST_SRCDIR}/_main"
    fi
    RESOLVE_SCRIPT="${WORKSPACE_DIR}/dependency/registry-resolve.sh"
else
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    WORKSPACE_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
    RESOLVE_SCRIPT="${WORKSPACE_DIR}/registry-resolve.sh"
fi
...
# The hermetic git bundle has no dashed `git-upload-pack`/`git-receive-pack`
# in libexec, but local/file transports spawn them by name via the shell.
# Shim them onto PATH so both the fixture setup and the script under test
# can talk to the local repo.
case "${GIT_BIN}" in
    /*) GIT_BIN_ABS="${GIT_BIN}" ;;
...
for cmd in upload-pack receive-pack; do
    printf '#!/bin/sh\nexec "%s" %s "$@"\n' "${GIT_BIN_ABS}" "${cmd}" > "${SHIM_DIR}/git-${cmd}"
    chmod +x "${SHIM_DIR}/git-${cmd}"
done
...
"${GIT_BIN}" -c init.defaultBranch=main init --bare "${BARE_REPO}" >/dev/null
