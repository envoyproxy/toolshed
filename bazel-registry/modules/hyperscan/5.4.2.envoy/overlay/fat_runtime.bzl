...
"""
    ...
    # Derive nm and objcopy paths from ar executable
    # Detect LLVM vs GNU toolchain based on ar path
    ar_dir = ar_executable.rpartition("/")[0]
    ar_basename = ar_executable.rpartition("/")[2]

    if "llvm" in ar_basename or "llvm" in ar_dir:
        # LLVM toolchain
        nm_executable = ar_dir + "/llvm-nm" if ar_dir else "llvm-nm"
        objcopy_executable = ar_dir + "/llvm-objcopy" if ar_dir else "llvm-objcopy"
    else:
        # GNU/GCC toolchain
        nm_executable = ar_dir + "/nm" if ar_dir else "nm"
        objcopy_executable = ar_dir + "/objcopy" if ar_dir else "objcopy"
